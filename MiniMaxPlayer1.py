...
#core capture/material weights that are tweaked often
CAPTURE_WEIGHT = 16.0             #capture differential is the primary score driver
PIECE_WEIGHT = 1.35               #remaining material should still influence value
MOBILITY_WEIGHT = 0.6             #raw legal move count provides baseline flexibility
CAPTURE_MOVE_WEIGHT = 0.8         #availability of immediate captures adds pressure
CENTER_WEIGHT = 0.35              #keeping stacks near the centre earns credit
STACK_WEIGHT = 0.45               #taller friendly stacks indicate initiative
LIBERTY_WEIGHT = 1.4              #more active origins reduce stalemate risk

#structural bonuses and penalties for board shape
EDGE_PENALTY_WEIGHT = 0.35        #if losing edge tug-of-war should hurt overall score
FROZEN_WEIGHT = 3.0               #immobile stacks represent significant liability
EDGE_OVERLOAD_WEIGHT = 1.8        #high rim occupancy must be punished aggressively
MEGA_STACK_PENALTY = 4.0          #giant towers tend to self-trap unless broken up
ORIGIN_REWARD = 2.0               #maintaining several distinct move origins is valuable
SPLIT_INCENTIVE = 1.5             #if in danger, nudge search toward splitting big stacks

#mobility safety thresholds for deeper search and evaluation
LOW_MOBILITY_THRESHOLD = 4        #four or fewer moves signals a fragile position
LOW_MOBILITY_WEIGHT = 4.5         #small mobility deficits should already cost points
MOBILITY_COLLAPSE_WEIGHT = 2.8    #collapsing mobility deserves a quadratic penalty
FORECAST_PENALTY_WEIGHT = 6.0     #lookahead warnings about low mobility must be heeded
FORECAST_BONUS_WEIGHT = 1.8       #forecasted breathing room earns a modest boost
FUTURE_ZERO_PENALTY = 60.0        #predicted stalemate next ply is nearly catastrophic
...
def getMove(state):
    #query all legal actions for current board
    ...
    player = state['Turn']
    #fewer moves, search deeper to see freeze traps
    depth = choose_depth(state, len(legal_moves))

    children = []
    #collect playable child states before running deeper search
    ...
    #capture-first ordering helps pruning later on
    ...
    for move, next_state in children:
        #evaluate child position via alpha-beta recursion
    ...
    #final decision for this ply
    return best_move


def choose_depth(state, legal_moves_count=None):
    #tune search depth dynamically from remaining pieces and mobility
    ...
    #clamp depth to valid range and ensure at least one ply
    return max(1, BASE_DEPTH + extra)


def alpha_beta(state, depth, alpha, beta, player):
    #classic alpha-beta minimax with move ordering
    legal_moves = GameRules.getAllLegalMoves(state)
    ...
    #sort moves so pruning hits likely captures/stacks first
    ...
    if maximizing:
        ...
        for move in ordered:
            ...
            if alpha >= beta:
                #pruning branch because opponent already has better option
                break
        if value == -math.inf:
    ...
    #minimizing branch mirrors the maximizing case
    value = math.inf
    for move in ordered:
        next_state = GameRules.playMove(state, move)
        if next_state is None:
            continue
        #recurse assuming opponent tries to minimize our value
        value = min(value, alpha_beta(next_state, depth - 1, alpha, beta, player))
        beta = min(beta, value)
        if beta <= alpha:
            #prune because maximizing side already secured better line
            break
    if value == math.inf:
...
def evaluate_state(state, player, legal_moves=None):
    ...
    #opponent metrics to compare against our mobility profile
    (
        opp_moves,
        opp_capture_moves,
        opp_frozen_weight,
    ...
    #compare our activity and structural stats versus opponent
    ...
    edge_ratio = (my_edge_weight / my_piece_total) if my_piece_total else 0.0  #share of our pieces hugging edge
    opp_edge_ratio = (opp_edge_weight / opp_piece_total) if opp_piece_total else 0.0  #opponent edge share

    #center and stack contrast reflect positional grip
    ...
    #blend our core heuristics: capture, material, activity, positioning
    score = (
        CAPTURE_WEIGHT * capture_diff          #prioritize current capture lead
        + PIECE_WEIGHT * piece_diff            #reward extra material on board
        + MOBILITY_WEIGHT * mobility_diff      #prefer having more legal moves
        + CAPTURE_MOVE_WEIGHT * capture_option_diff  #value pressure from available captures
        + CENTER_WEIGHT * center_diff          #interior control matters
        + STACK_WEIGHT * stack_diff            #taller friendly stacks threaten tempo
        + LIBERTY_WEIGHT * liberty_diff        #more active origins keep flexibility
    )

    score += mobility_pressure                      #dynamic mobility penalties/bonuses
    score += FROZEN_WEIGHT * frozen_diff             #punish own frozen stacks versus opponent
    score += EDGE_PENALTY_WEIGHT * edge_diff         #offset edge exposure differential
    #threshold forces us to reduce over-reliance on rim stacks
    ...
    #aggregated heuristic value for current player perspective
    return score


def terminal_score(state, player):
    #derive final winner purely from capture totals
    my_capture = capture_total(state, player)
...
def order_children(state, children, player):
    def priority(item):
        ...
        #prefer capture moves, bigger gains, and larger stacks first
        return (-is_capture, -immediate_gain, -stack_size)

    #search best looking moves first, alpha-beta cuts more branches
...
def order_moves_simple(state, moves):
    def priority(move):
        is_capture = 1 if move['Direction'] in CARDINALS else 0
        stack_size = GameRules.getPieces(state['Board'], move['Row'], move['Col'])
        #rank by capture potential then larger originating stacks
        return (-is_capture, -stack_size)

    #helper shares ordering logic for other callers
    return sorted(moves, key=priority)


def mobility_profile(state, player):
    #summarise mobility, edge load, and stack risks for a player
    ...
    for mv in moves:
        if mv['Direction'] in CARDINALS:
            capture_moves += 1
        #record squares that produced moves so we know active origins
        active_origins.add((mv['Row'], mv['Col']))

    #evaluate board features for the player copy
    board = probe['Board']
    frozen_weight = 0      #total pieces sitting on origins without moves
    edge_weight = 0        #cumulative stack size on rim squares
    piece_total = 0        #total material counted for ratio calculations
    max_stack = 0          #tallest stack we encounter while scanning
    for r in range(6):
        for c in range(6):
            ...
            if pieces == 0:
                continue
            piece_total += pieces       #accumulate material for later ratios
            if pieces > max_stack:
                max_stack = pieces      #maintain maximum stack height seen so far
            if r in (0, 5) or c in (0, 5):
                #accumulate rim exposure to penalize edge-heavy distributions
                edge_weight += pieces
            if (r, c) not in active_origins:
                #if a stack cannot move this turn, treat those pieces as frozen risk
                frozen_weight += pieces

    #return tuple: move count, captures, frozen load, edge load, origins, total pieces, tallest stack
    return (
        len(moves),
        capture_moves,
        frozen_weight,
...
#forecast sampling limits stay near this helper
...
def forecast_mobility_swing(state, player, legal_moves):
    ...
    if state['Turn'] == player:
        #simulate our best-looking moves first for faster pruning
        ...
        for move in ordered_moves[:FORECAST_MOVE_LIMIT]:
            ...
            min_future = math.inf
            max_future = -math.inf
            #opponent replies are likewise sorted for relevance
...
def central_control(state, player):
    #favour pieces nearer the centre weighted by stack size
    board = state['Board']
    score = 0.0
    #iterate every square to accumulate centre influence
    for r in range(6):
        for c in range(6):
            if GameRules.color(r, c) != player:
                continue
            ...
            if pieces == 0:
                continue
            #compute Manhattan distance from centre (2.5, 2.5)
            dist = abs(2.5 - r) + abs(2.5 - c)
            #closer to board centre yields larger bonus (3 - distance)
            score += pieces * (3.0 - dist)
    return score


def max_stack_height(state, player):
    #track maximum stack height for tempo/combat potential
    board = state['Board']
    best = 0
    for r in range(6):
        for c in range(6):
...
def count_pieces(state, player):
    #sum all stack sizes on player's colour squares
    board = state['Board']
    total = 0
    for r in range(6):
        for c in range(6):
...
def capture_total(state, player):
    #capture counts stored separately for each colour
    if player == 'Dark':
